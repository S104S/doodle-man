...
fn handle_connection(mut stream: TcpStream) -> String {
    ...
    if buffer.starts_with(get) {
        ...
        let buffer_str = std::str::from_utf8(&buffer[0..100]).expect("Error getting part of buffer.");
...
println!("OS being used: {}", env::consts::OS);
        ...
        let final_user_auth_uri =
        format!("{}?client_id={}&response_type={}&redirect_uri={}&scope=chat:read+chat:edit",
        ...
        for stream in web_server.incoming() {
            ...
            if token != "No access token" {
                ...
                if !codes_used.iter().any(|r| r == &token) {
                    ...
                    let secret = "ziocnlyb84z38qp73jeq855k06cdaw";
